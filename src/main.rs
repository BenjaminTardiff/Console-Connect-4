...
impl Board {
    ...
    fn check_for_positive_diagonal(&self, start_index: usize) -> bool {
        let mut color:Option<Piece>;
    ...
    fn check_for_negative_diagonal(&self, start_index: usize) -> bool {
        let mut color: Option<Piece>;
        let mut diagonal_counter:u8;
        if start_index < 21 || {6 < start_index && start_index < 10} || {13 < start_index && start_index < 17} || {20 < start_index && start_index < 24} || {27 < start_index && start_index < 31} || {34 < start_index && start_index < 38} {}
        else {
            for i in 0..2 {
                diagonal_counter = 0;
                if i == 0 {
                ...
                for j in 0..4 {
                    if self.data[start_index - 8 * j] == color {
                        diagonal_counter += 1;
                    }
                    if diagonal_counter == 4 {
                        return true
    ...
    fn check_for_win(&self) -> bool {
        for i in 0..42 {
            if self.check_for_column(i) || self.check_for_row(i) || self.check_for_positive_diagonal(i) || self.check_for_negative_diagonal(i) {
                return true
            }
        }
        return false;
